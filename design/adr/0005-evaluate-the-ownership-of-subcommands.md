# ADR-0005: Evaluate ownership of subcommands
...
Should `aictl eval` be a formal CLI command, or only full/optional (§4.6 Evaluation optional)? ---
...
This ADR solidifies "the ownership of the evaluation subcommand" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
...
- Associated architecture documents §4.6 (as a basis for decision-making and a source of consistency verification).
