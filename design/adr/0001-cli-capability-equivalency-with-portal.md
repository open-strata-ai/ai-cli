# ADR-0001: CLI and Portal Capability Equivalence
...
Does the CLI need to expose everything that the portal can do (such as dual-writing canary vector libraries)? Or does the CLI only do the subset commonly used by developers?
...
This ADR solidifies the "capability equivalence of CLI and portal" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
