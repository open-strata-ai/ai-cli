# ADR-0004: Offline/Air Isolated Environment
...
How does the CLI pull metacang profiles and bom (local cache policy) when there is no external network?
...
This ADR solidifies the "offline/air isolation environment" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §14 open issue and is still open.
